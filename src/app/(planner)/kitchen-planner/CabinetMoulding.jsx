...
export default function CabinetMoulding({
  ...
  const mouldingSize = 0.001 / 1000 // 4mm x 4mm default moulding size
  const mouldingRadius = 0.8 / 1000 // radius for rounding the moulding (half of mouldingSize by default)
  const frameInset = 4 / 1000 // 4mm inset for the frame
  ...
  const holeBoundaries = useMemo(() => {
    ...
    if (numHoles === 1) {
      ...
      const availableHeight = totalHoleHeight - numDividers * panelThickness
      ...
      for (let i = 0; i < numHoles; i++) {
        ...
        currentBottom = currentTop + panelThickness
      }
    } else if (numHoles === 3) {
