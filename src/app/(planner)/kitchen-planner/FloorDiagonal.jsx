        ...
        <meshStandardMaterial
          ...
          onBeforeCompile={(shader) => {
            ...
            shader.fragmentShader = shader.fragmentShader.replace(
              ...
              // Rotated checkerboard function
              vec3 getCheckerboard(vec3 pos, float size, vec3 color1, vec3 color2) {
                float angle = 0.78539816339; // 45 degrees in radians
                mat2 rot = mat2(cos(angle), -sin(angle), sin(angle), cos(angle)); // 45 degrees rotation
                vec2 posR = pos.xy * rot;
                // Use x and z for checkerboard pattern (x and y in object space due to rotation)
                float x = posR.x;
                float z = posR.y;
                
                // Create checkerboard pattern
                float checker = mod(floor(x / size) + floor(z / size), 2.0);
